...
#include <mach/dev-hawaii_vivaltonfc3g_bt.h>
#include <mach/dev-hawaii_vivaltonfc3g_camera.h>
#include <mach/dev-hawaii_vivalto3g_nfc.h>
#include <mach/dev-hawaii_vivaltonfc3g_tsp.h>
...
static void headset_enable_external_ldo(void)
{
	...
	micbias_ldo = regulator_get(NULL, "gpldo1_uc");
	if (IS_ERR_OR_NULL(micbias_ldo)) {
		printk("%s: ERR/NULL getting MICBIAS LDO\n", __func__);
		micbias_ldo = NULL;
		return ;
	}
	regulator_set_voltage(micbias_ldo, 2600000, 2600000);

	if(!regulator_is_enabled(micbias_ldo)){
		ret=regulator_enable(micbias_ldo);
		if(ret < 0)
			return;
	}
	else
		printk("%s: micbias_ldo is already enabled\n", __func__);

	regulator_put(micbias_ldo);
}
...
static void headset_disable_external_ldo(void)
{
	...
	if(regulator_is_enabled(micbias_ldo))
		...
		printk("%s: micbias_ldo is already disabled\n", __func__);
...
#ifdef CONFIG_BACKLIGHT_KTD2801_VIVALTONFC3G
...
#endif /* CONFIG_BACKLIGHT_KTD2801_VIVALTONFC3G */
...
#if	defined(CONFIG_BACKLIGHT_KTD259B) || defined(CONFIG_BACKLIGHT_KTD2801) || defined(CONFIG_BACKLIGHT_KTD2801_VIVALTONFC3G)
...
MACHINE_START(HAWAII, "hawaii_ss_vivaltonfc3g")
