...
static int wifi_pin_configuration()
{
	int rc = 0;

	printk(KERN_ERR "%s:ENTRY\n", __func__);

	PRINT_INFO("wifi_pin_configuration:%d, wake:%d \n",devtreeWifiParms.reset,devtreeWifiParms.host_wake);

	rc = gpio_request(devtreeWifiParms.reset, "wl_reset");
	if (rc < 0) {
		PRINT_ERR("unable to request reset GPIO pin %d\n",devtreeWifiParms.reset);
		goto pin_config_err;
	}
	printk(KERN_ERR "%s: RESET=%x\n", __func__, devtreeWifiParms.reset);
	PRINT_INFO("current value of reset GPIO: %d\n", devtreeWifiParms.reset);
	gpio_direction_output(devtreeWifiParms.reset, 0);
	

	rc = gpio_request(devtreeWifiParms.host_wake, "wl_host_wake");
	if (rc < 0) {
		PRINT_ERR("unable to request wake GPIO pin %d\n",devtreeWifiParms.host_wake);
		
		gpio_free(devtreeWifiParms.reset);
		goto pin_config_err;
	}
	gpio_direction_input(devtreeWifiParms.host_wake);
	rc = irq_set_irq_type(gpio_to_irq(devtreeWifiParms.host_wake), IRQ_TYPE_EDGE_RISING);
	if (rc < 0) {
		PRINT_ERR("unable to set irq type for GPIO pin %d\n",devtreeWifiParms.host_wake);
		gpio_free(devtreeWifiParms.reset);
		gpio_free(devtreeWifiParms.host_wake);
		goto pin_config_err;
	}
	printk(KERN_ERR "%s: HOST_WAKE=%x\n", __func__, devtreeWifiParms.host_wake);
...
static int bcm_sdiowl_init(int onoff)
{
    ...
    gpio_set_value(devtreeWifiParms.reset, 1);
	msleep(200);
	
	printk("WIFI_REG_ON: %d",gpio_get_value(devtreeWifiParms.reset));    	
...
static void bcm_sdiowl_term(void)
{
	...
	gpio_set_value(devtreeWifiParms.reset,0);
}
...
extern void mmc_ctrl_clk(struct mmc_host *host, bool onoff);
...
static int hawaii_wifi_power(int on)
{
	...
	mmc_ctrl_clk((struct mmc_host *)wifi_mmc_host, on);
