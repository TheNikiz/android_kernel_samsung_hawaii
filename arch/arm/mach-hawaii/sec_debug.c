...
#ifdef CONFIG_BACKLIGHT_KTD2801_VIVALTODS5M
extern void lcd_backlight_control(int num);
#endif
...
static void sec_debug_set_upload_cause(enum sec_debug_upload_cause_t type)
{
	...
	__raw_writel(type, BROADCOM_INFORM3);
	__raw_writel(type, BROADCOM_INFORM4);
	__raw_writel(type, BROADCOM_INFORM6);
...
void sec_debug_check_crash_key(unsigned int code, int value)
{
	...
	if(code == VOLUME_DOWN || code == KEY_POWER)
		pr_info("%s %d %d\n", __func__, code, value);
