...
#ifdef CONFIG_HAWAII_BCM21663_CPU_TABLE
	kona_freq_tbl[1].cpu_freq = 600000;
	kona_freq_tbl[2].cpu_freq = 800000;
#else
...
#if defined(CONFIG_PSTORE_RAM_ADDRESS) && defined(CONFIG_PSTORE_RAM_SIZE)
void __init pstore_reserve(void) 
{
	ulong address = CONFIG_PSTORE_RAM_ADDRESS;
	ulong size = CONFIG_PSTORE_RAM_SIZE;
	int ret = -EINVAL;

	ret = memblock_reserve(address, size);

	pr_info("%s:%d: 0x%lx@0x%lx %sreserved ret=%d\n",
			__func__, __LINE__, size, address, ret < 0 ? "not " : "", ret);
}
#endif
