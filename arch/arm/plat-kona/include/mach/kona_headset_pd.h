...
struct kona_headset_pd {
	...
	unsigned int (*accessory_adc_values)[2];
	int gpio_debounce;
	int jack_insertion_settle_time;
	int jack_remove_settle_time;
	int adc_check_offset;
	int comp1_threshold;
	int comp2_threshold;
	int use_external_micbias;
	void (*enable_external_ldo)(void);
	void (*disable_external_ldo)(void);
	char *audio_ldo_id;
};
