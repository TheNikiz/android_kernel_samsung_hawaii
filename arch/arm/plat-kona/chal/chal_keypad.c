...
/******************************************************************************
 *
 * Function Name: chal_keypad_decode_pressed_key
 *
 * Description: Take one key status register value and decode all
 *              pressed key contained in it and add them to the Key Event FIFO
 *
 * Parameters: none
 *
 ******************************************************************************/
cUInt32 chal_keypad_decode_pressed_key(CHAL_HANDLE *handle,
						CHAL_KEYPAD_REGISTER_SET_t *
						regState,
						CHAL_KEYPAD_KEY_EVENT_LIST_t
						keyEvents)
{
	cUInt32 bc1, bc2;
	cUInt32 ssr0 = regState->ssr0, ssr1 = regState->ssr1;
	cUInt32 numKeyEvents = 0;
	CHAL_KEYPAD_EVENT_t *keyEvent;
	cUInt32 mask, i, firstbit;
	CHAL_KEYPAD_DEVICE_t *pKpdDev = (CHAL_KEYPAD_DEVICE_t *) handle;

	if (pKpdDev->pullUpMode) {
		ssr0 = ~ssr0;
		ssr1 = ~ssr1;
	}
	...
	for (i = 0; i < bc1; i++) {
		keyEvent = &keyEvents[numKeyEvents];
		firstbit = mask & (-mask);
		keyEvent->keyId = bitNumber(firstbit);
		keyEvent->keyAction = CHAL_KEYPAD_KEY_PRESS;
		mask &= (~firstbit);
		(numKeyEvents)++;
	}
	...
	for (i = 0; i < bc2; i++) {
		keyEvent = &keyEvents[numKeyEvents];
		firstbit = mask & (-mask);
		keyEvent->keyId = bitNumber(firstbit) + 32;
		keyEvent->keyAction = CHAL_KEYPAD_KEY_PRESS;
		mask &= (~firstbit);
		(numKeyEvents)++;
	}
