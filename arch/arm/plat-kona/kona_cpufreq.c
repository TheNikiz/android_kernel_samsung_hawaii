...
#define KONA_FREQ_NUM_LOG_ENTRIES	256

static int kcf_debug = 0;
static unsigned int bitmap_min_limit;
spinlock_t bitmap_minlimit_lock;
...
enum kona_freq_log_states {
	KONA_FREQ_LOG_NEW_TARGET_FREQ = 0x1,
	KONA_FREQ_LOG_AFTER_NOTIFY_PRE_CHANGE = 0x2,
	KONA_FREQ_LOG_AFTER_DFS_UPDATE = 0x4,
	KONA_FREQ_LOG_FREQ_CHANGE_FAIL = 0x8,
	KONA_FREQ_LOG_AFTER_STATS_UPDATE = 0x10,
	KONA_FREQ_LOG_AFTER_IRQ_ENABLE = 0x20,
	KONA_FREQ_LOG_AFTER_CPUFREQ_UPDATE_LP = 0x40,
	KONA_FREQ_LOG_AFTER_NOTIFY_POST_CHANGE = 0x80,
};

static struct kona_freq_log *kona_freq_log_buf;
static dma_addr_t kona_freq_log_buf_phys;
static u16 buf_idx = 0;
static u16 buf_count = 0;
				  ...
				  unsigned int relation)
{
	...
	/* Log buffer index handling */
	idx = buf_idx++;
	if (buf_idx >= KONA_FREQ_NUM_LOG_ENTRIES)
		buf_idx = 0;

	/* Track ID and frequencies */
	kona_freq_log_buf[idx].entry_num = buf_count++;
	kona_freq_log_buf[idx].old_freq = freqs.old;
	kona_freq_log_buf[idx].new_freq = freqs.new;
	...
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_NEW_TARGET_FREQ;

	cpufreq_notify_transition(policy, &freqs, CPUFREQ_PRECHANGE);

	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_NOTIFY_PRE_CHANGE;
	...
	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_DFS_UPDATE;

	if (unlikely(ret)) {
		kcf_dbg("%s: cpu freq change failed : %d\n", __func__, ret);
		kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_FREQ_CHANGE_FAIL;
	}
	...
	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_STATS_UPDATE;

	local_irq_enable();

	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_IRQ_ENABLE;
...
#endif
	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_CPUFREQ_UPDATE_LP;

	cpufreq_notify_transition(policy, &freqs, CPUFREQ_POSTCHANGE);

	/* comment here */
	kona_freq_log_buf[idx].state_flag |= KONA_FREQ_LOG_AFTER_NOTIFY_POST_CHANGE;
...
static unsigned int bitmap_min_limit_set(unsigned int bitmap)
{
	spin_lock(&bitmap_minlimit_lock);
	bitmap_min_limit |= (0x1 << bitmap);
	spin_unlock(&bitmap_minlimit_lock);
	kcf_dbg("[%s, %d]: min limit bitmap:%u \n", __func__, __LINE__, bitmap_min_limit);
	return bitmap_min_limit;
}

static unsigned int bitmap_min_limit_clear(unsigned int bitmap)
{
	spin_lock(&bitmap_minlimit_lock);
	bitmap_min_limit &= ~(0x1 << bitmap);
	spin_unlock(&bitmap_minlimit_lock);
...
int cpufreq_update_universal_lmt_req(unsigned int bitmap, int val, int lmt_typ)
{
	
	kcf_dbg("%s: bitmap:%u val:%d type:%s \n", __func__, bitmap, val, lmt_typ ? "Max" : "Min");
	
	if(lmt_typ == MAX_LIMIT)
	{
		unsigned int min = 0;

		get_cpufreq_limit(&min, MIN_LIMIT);
		if((int)min > val)
			cpufreq_update_usr_lmt_req(val, MIN_LIMIT);

		cpufreq_update_usr_lmt_req(val, MAX_LIMIT);
	}
	else if(lmt_typ == MIN_LIMIT)
	{
		if(val == RESTORE_MIN_CLOCK) {
			if(!bitmap_min_limit_clear(bitmap))
				cpufreq_update_usr_lmt_req(get_cpu_freq_from_opp(PI_OPP_ECONOMY), MIN_LIMIT);

		} else {
			unsigned int max_clock = 0;

			bitmap_min_limit_set(bitmap);

			get_cpufreq_limit(&max_clock, MAX_LIMIT);
			if( val == BOOST_MIN_CLOCK )
				val = (int)max_clock;
			else
				val = (val > (int)max_clock) ? (int)max_clock:val;
...
EXPORT_SYMBOL(cpufreq_update_universal_lmt_req);
...
{														\
	...
	if (strict_strtol(buf, 10, &val))					\
		return -EINVAL;									\
														\
	cpufreq_update_universal_lmt_req(					\
		BITMAP_USER_SPACE_DVFS, val, lmt_typ);			\
														\
	return n;											\
}
...
static void minlimit_early_suspend(struct early_suspend *h)
{
	kcf_dbg("early suspend: bitmap_min(%u)\n", bitmap_min_limit);
	bitmap_min_limit = 0;
}

static void minlimit_late_resume(struct early_suspend *h)
{
	kcf_dbg("late_resume: min limit\n");
}

static struct early_suspend minlimit_early_suspend_desc = {
	.level = EARLY_SUSPEND_LEVEL_STOP_DRAWING,
	.suspend = minlimit_early_suspend,
	.resume = minlimit_late_resume,
};
#else

int cpufreq_update_universal_lmt_req(int val, int lmt_typ)
{	
	return 0;
}
EXPORT_SYMBOL(cpufreq_update_kernel_lmt_req);
...
static int cpufreq_drv_probe(struct platform_device *pdev)
{
	...
	if ((sizeof(struct kona_freq_log) * KONA_FREQ_NUM_LOG_ENTRIES) > PAGE_SIZE) {
		kcf_dbg("%s: KONA_FREQ_NUM_LOG_ENTRIES too large\n",
				__func__);
		return -ENOMEM;
	}

	kona_freq_log_buf = dma_alloc_coherent(NULL, PAGE_SIZE,
					&kona_freq_log_buf_phys, GFP_ATOMIC);

	if (!kona_freq_log_buf) {
		kcf_dbg("%s: dma_alloc_coherent failed for kona_cpufreq log buffer\n",
				__func__);
		return -ENOMEM;
	}

	printk("kona_freq_log_buf v:0x%x, p:0x%x\n", (u32)kona_freq_log_buf,
			(u32)kona_freq_log_buf_phys);
...
#ifdef CONFIG_KONA_CPU_FREQ_LIMITS
	register_early_suspend(&minlimit_early_suspend_desc);
#endif
