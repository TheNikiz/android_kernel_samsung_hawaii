...
#define MAX_COMPARE_LOAD_UPDATE_WAIT_CYCLE  10
...
struct reg_buf {
	...
	unsigned int cnt; /* counter lsw value */
};
...
//#ifdef CONFIG_KONA_TIMER_REG_LOG_DBG
#if 1
#define writel_dbg(v, c, cpu, j) \
	do { \
		...
		kr->buf[kr->curr].cnt = __get_counter((void __iomem *)((unsigned int)(c)&0xffffff00)); \
...
int __init kona_timer_modules_init(void)
{
	...
	for_each_possible_cpu(i) {
		struct kona_timer_reg *kr = &per_cpu(kona_timer_reg_seq, i);
		spin_lock_init(&kr->lock);
	}
...
int kona_timer_set_match_start(struct kona_timer *kt, unsigned long load)
{
	...
	if (kt->expire >= 0xFFFEFC00) {
		...
		if ((next_lsw >= 0) && ((s32)(lsw) < 0)) {
			__disable_channel(ktm->reg_base, kt->ch_num);
			writel_dbg(next_lsw + MIN_KONA_DELTA_CLOCK,
			       ktm->reg_base + KONA_GPTIMER_STCM0_OFFSET +
			       (kt->ch_num * 4), cpu, jiffies);
#ifdef CONFIG_GP_TIMER_COMPARATOR_LOAD_DELAY
			__wait_for_compare_val_sync(ktm->reg_base, kt->ch_num);
#endif
			reg &= ~KONA_GPTIMER_STCS_TIMER_MATCH_MASK;
			...
			writel_dbg(reg, ktm->reg_base + KONA_GPTIMER_STCS_OFFSET
			           , cpu, jiffies);
		}
	}
out:
...
static void kona_dbg_get_timer_state(int context)
{
	...
	spin_lock_irqsave(&ktm->lock, flags);
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCS_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCS = %08x\n", reg);
	kona_timer_state[context].stcs = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCLO_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCLO = %08x\n", reg);
	kona_timer_state[context].stclo = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCHI_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCHI = %08x\n", reg);
	kona_timer_state[context].stchi = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCM0_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCM0 = %08x\n", reg);
	kona_timer_state[context].stcm0 = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCM1_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCM1 = %08x\n", reg);
	kona_timer_state[context].stcm1 = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCM2_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCM2 = %08x\n", reg);
	kona_timer_state[context].stcm2 = reg;
	reg = readl(ktm->reg_base + KONA_GPTIMER_STCM3_OFFSET);
	printk(KERN_ERR "KONA_GPTIMER_STCM3 = %08x\n", reg);
	kona_timer_state[context].stcm3 = reg;
	spin_unlock_irqrestore(&ktm->lock, flags);
}
...
#endif
	kona_dump_timer_regs(true, HUB_TIMER);
	kona_dump_timer_regs(true, SLAVE_TIMER);
#ifdef CONFIG_ARCH_JAVA
	kona_dump_timer_regs(true, CORE_TIMER);
#endif
}
...
static inline void __disable_channel(void __iomem *reg_base, int ch_num)
{
	...
	writel_dbg(reg,	reg_base + KONA_GPTIMER_STCS_OFFSET, cpu,
			jiffies);
