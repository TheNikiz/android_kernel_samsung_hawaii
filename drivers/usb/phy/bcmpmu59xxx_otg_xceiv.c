...
#ifdef CONFIG_RT8973
extern uint8_t rt8973_get_otg_status(void);
#endif
...
static int bcmpmu_otg_xceiv_set_vbus(struct usb_otg *otg, bool enabled)
{
	...
	dev_info(xceiv_data->dev, "VBUS enable=%d\n", enabled);
...
#ifdef CONFIG_RT8973
        data = rt8973_get_otg_status();
#endif

	dev_info(xceiv_data->dev, "id_gnd check; %s\n",
		(data == 1) ? "OTG attached" : "OTG detached");

	id_gnd = (data == 1) ? true : false;
	return id_gnd;
}
...
static void bcmpmu_otg_xceiv_id_change_handler(struct work_struct *work)
{
	...
	if (id_gnd || id_rid_a || id_rid_c) {
		bcm_hsotgctrl_phy_deinit();
		xceiv_data->otg_xceiver.phy.state = OTG_STATE_UNDEFINED;
		atomic_notifier_call_chain(&xceiv_data->otg_xceiver.phy.notifier,
			USB_EVENT_ID, NULL);
