...
static long  mtpg_ioctl(struct file *fd, unsigned int code, unsigned long arg)
{
	...
	case SEND_RESET_ACK:
		req = dev->ep0req;
		if (!req) {
			printk(KERN_DEBUG "[%s] ep0req is NULL\n", __func__);
			return -EAGAIN;
		}
		req->length = 0;
...
mtpg_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (dev->ep0req)
		mtpg_request_free(dev->ep0req, dev->cdev->gadget->ep0);
...
mtpg_function_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	mtpg->ep0req = mtpg_request_new(c->cdev->gadget->ep0,
		USB_PTPREQUEST_GETSTATUS_SIZE+1);
	if (unlikely(!mtpg->ep0req))
		goto out;
				...
				const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		dev->ep0req->complete = req->complete;
		break;
