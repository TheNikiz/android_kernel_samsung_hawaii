...
static int gator_meminfo_func(void *data)
{
	struct sysinfo info;
	int i, len;
	unsigned long long value,avail;
	...
	for (;;) {
		...
		si_meminfo(&info);
		avail=global_page_state(NR_FREE_PAGES) + global_page_state(NR_FILE_PAGES);
		for (i = 0; i < MEMINFO_TOTAL; i++) {
			if (meminfo_enabled[i]) {
				switch (i) {
				case MEMINFO_MEMFREE:
					value = avail* PAGE_SIZE;
					break;
				case MEMINFO_MEMUSED:
					...
					meminfo_buffer[len++] = (info.totalram - avail) * PAGE_SIZE;
					// Clear pid
					meminfo_buffer[len++] = 1;
