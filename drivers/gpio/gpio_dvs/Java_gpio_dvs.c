...
static unsigned char analyze_gpio_lowhigh(gpioInitStatusTest_t *gpiostatus, unsigned char temp_io);
...
unsigned long gpio_invalue[4];
unsigned long gpio_outvalue[4];
...
unsigned char analyze_gpio_pupd(gpioInitStatusTest_t *gpiostatus)
{
	...
	if((gpiostatus->regvalue&GPIO_NP_BIT_MASK)==0x00)
		result = GDVS_PUPD_NP;	 /* NP */
	else if((gpiostatus->regvalue&GPIO_PD_BIT_MASK)==0x40)
		result = GDVS_PUPD_PD;	 /* PD */
	else if((gpiostatus->regvalue&GPIO_PU_BIT_MASK)==0x20)
		result = GDVS_PUPD_PU;	 /* PU */
	else
		result = GDVS_PUPD_ERR;	/* error */

	return result;
}

unsigned char analyze_gpio_lowhigh(gpioInitStatusTest_t *gpiostatus, unsigned char temp_io)
{
	unsigned char result = 0;	/* 1:HIGH, 0:LOW */

	if (temp_io == GDVS_IO_OUT)
	{
		if (gpiostatus->gpiohl_out == 0x01)
			result = GDVS_HL_H;	/* HIGH */
		else
			result = GDVS_HL_L; /* LOW */
	}
	else if (temp_io == GDVS_IO_IN)
	{
		if (gpiostatus->gpiohl_in == 0x01)
			result = GDVS_HL_H;	/* HIGH */
	else
			result = GDVS_HL_L; /* LOW */
	}
	else
		result = GDVS_HL_UNKNOWN;	/* Unknown */
	...
	gpioInitStatusTest_t *gpiostatus, unsigned char type)
{
	...
	if (type == TYPE1) {
		...
		if ((gpiostatus->regvalue &
			...
			if ((gpiostatus->gpiodirection &
				...
				temp_io = GDVS_IO_IN;	/* GPIO_IN */
			else
				temp_io = GDVS_IO_OUT;	/* GPIO_OUT */
		}
		/* It's FUNC!! */
			else
			temp_io = GDVS_IO_FUNC;		/* FUNC */
	} else if (type == TYPE2) {
		/* It's GPIO!! */
		if ((gpiostatus->regvalue &
	...
	/* 4 bit (0:LOW, 1:HIGH) */
	temp_lh = analyze_gpio_lowhigh(gpiostatus, temp_io);
...
static void check_gpio_status(unsigned char phonestate)
{
	...
	gpio_outvalue[0] = BRCM_READ_REG(base_addr,  GPIO_GPOR0);
	gpio_outvalue[1] = BRCM_READ_REG(base_addr,  GPIO_GPOR1);
	gpio_outvalue[2] = BRCM_READ_REG(base_addr,  GPIO_GPOR2);
	gpio_outvalue[3] = BRCM_READ_REG(base_addr,  GPIO_GPOR3);

	gpio_invalue[0] = BRCM_READ_REG(base_addr,  GPIO_GPIR0);
	gpio_invalue[1] = BRCM_READ_REG(base_addr,  GPIO_GPIR1);
	gpio_invalue[2] = BRCM_READ_REG(base_addr,  GPIO_GPIR2);
	gpio_invalue[3] = BRCM_READ_REG(base_addr,  GPIO_GPIR3);

	// platform customizing code about gpio H/L
	for (i = 0; i < 102; i++)
		check_gpiostate[i].gpionum = i;
	for (i = 102; i < AP_GPIO_COUNT; i++)
		check_gpiostate[i].gpionum = i+8;

	for (i = 0; i < AP_GPIO_COUNT; i++)
	{
		#define TEMP_GPIONUM (check_gpiostate[i].gpionum)
		check_gpiostate[i].gpiohl_out = 
			(((gpio_outvalue[TEMP_GPIONUM/32] >> (TEMP_GPIONUM%32)) & 0x01) ? 
				0x01 : 0x00);
		check_gpiostate[i].gpiohl_in = 
			(((gpio_invalue[TEMP_GPIONUM/32] >> (TEMP_GPIONUM%32)) & 0x01) ? 
				0x01 : 0x00);
	}
	
	//for (i = 0; i < AP_GPIO_COUNT; i++)
	//	check_gpiostate[i].state = gpio_get_value(check_gpiostate[i].gpionum);
#if 0
	for (i = 0; i < AP_GPIO_COUNT; i++)
...
void setgpio_for_sleepdebug(int gpionum, uint16_t  io_pupd_lh)
{
	...
	if (temp_io == GDVS_IO_IN) {
		...
		temp_gpio_reg = temp_gpio_reg | (0x00000000 | temp_data);
	...
	/* in case of 'OUTPUT', set L/H */
		...
		gpio_set_value(check_gpiostate[gpionum].gpionum, temp_lh);
	}
