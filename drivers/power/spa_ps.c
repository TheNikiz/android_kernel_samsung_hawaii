...
static int spa_batt_get_property( struct power_supply *batt, enum power_supply_property property, union power_supply_propval *propval)
{
		...
		switch (property) {
				...
				case POWER_SUPPLY_PROP_VOLTAGE_NOW:
						propval->intval = spa_ps_iter->state.voltage_now * 1000;
						break;
				...
				case POWER_SUPPLY_PROP_ONLINE:
						propval->intval = spa_ps_iter->state.batt_online;
						break;
...
static int spa_batt_set_property( struct power_supply *batt, enum power_supply_property property, const union power_supply_propval *propval)
{
		int ret = 0;
		...
		switch (property) {
				...
				case POWER_SUPPLY_PROP_VOLTAGE_NOW:
						spa_ps_iter->state.voltage_now = propval->intval;
						break;
				...
				case POWER_SUPPLY_PROP_ONLINE:
						spa_ps_iter->state.batt_online = propval->intval;
						break;
...
static int spa_ac_set_property( struct power_supply *ac, enum power_supply_property property, const union power_supply_propval *propval)
{
		int ret = 0;
		struct spa_ps *spa_ps_iter = container_of(ac,struct spa_ps, ac);
...
static int spa_usb_set_property( struct power_supply *usb, enum power_supply_property property, const union power_supply_propval *propval)
{
		int ret = 0;
		struct spa_ps *spa_ps_iter = container_of(usb,struct spa_ps, usb);
