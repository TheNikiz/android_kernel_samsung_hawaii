...
static struct device_attribute ss_batt_ext_attrs[]=
{
	...
	__ATTR(update, 0644, ss_batt_ext_attrs_show, ss_batt_ext_attrs_store),
};
...
#endif
		break;
	case SS_BATT_VOL_UPDATE:
	{
		pr_spa_dbg(LEVEL3, "%s voltage_update", __func__);
		if(spa_power_iter)
		{
			cancel_delayed_work_sync(&spa_power_iter->battery_work);
			spa_power_iter->new_gathering.temperature = 1;
...
#if defined(SPA_BACKCHARGING_CONDITION)
...
static void spa_update_batt_info(struct spa_power_desc *spa_power_iter, unsigned int prop)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		pr_spa_dbg(LEVEL3, "%s POWER_SUPPLY_PROP_VOLTAGE_NOW", __func__);
		spa_power_iter->batt_info.voltage_now = spa_get_batt_voltage(spa_power_iter);
		value.intval= spa_power_iter->batt_info.voltage_now;
		pr_spa_dbg(LEVEL4, "%s : voltage = %d\n", __func__, spa_power_iter->batt_info.voltage_now);
...
static void spa_batt_work(struct work_struct *work)
{
	...
	spa_update_power_supply_battery(spa_power_iter, POWER_SUPPLY_PROP_HEALTH);
	spa_update_power_supply_battery(spa_power_iter, POWER_SUPPLY_PROP_PRESENT);
