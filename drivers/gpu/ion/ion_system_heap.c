...
#ifdef CONFIG_ION_DIRECT_RECLAIM_REDUCTION
/*
 * COMMENT (yh77.cho)
 * ------------------
 * Heat is low memory device (512 MB).
 * So, order 4, 8 is too large, I think.
 *
 * Here is trade-off
 * "Direct reclaim" vs. "Fragmented heap".
 * "Fragmented heap" mainly can be handled by SMMU (That is, HW covers it).
 * But, "Direct reclaim" is SW area.
 * So, in my opinion, "Direct reclaim" is more critical.
 * Especially, in terms of worst case response time,
 *   "Direct reclaim" is evil enough.
 *
 * So, low_order_gfp_flags are used only for order 0.
 */
static const int MAX_LOW_ORDER = 0;
static const unsigned int orders[] = {8, 4, 2, 1, 0};
#else /* CONFIG_ION_DIRECT_RECLAIM_REDUCTION */
static const int MAX_LOW_ORDER = 4;
static const unsigned int orders[] = {8, 4, 0};
#endif /* CONFIG_ION_DIRECT_RECLAIM_REDUCTION */
		...
		unsigned long len)
{
	...
	for_each_sg(table->sgl, sg, table->nents, i) {
		...
		if ((curr_off < end_off) && (curr_end > offset)) {
			...
			l_off -= curr_off;
			arm_dma_ops.sync_single_for_device(NULL,
				pfn_to_dma(NULL, page_to_pfn(page)) + l_off,
