...
#include "./firmware/IST30xxB_FW_VivaltoNFC3G5M_0X0035.h"

#if IST30XX_MULTIPLE_TSP
#include "./firmware/IST30xxB_FW_VivaltoNFC3G5M_0X0022.h"
#endif
...
int ist30xxb_read_compare(struct i2c_client *client, u8 *buf, int page_size)
{
	int ret = 0;
	int i, j;
	int len = EEPROM_PAGE_SIZE / IST30XX_DATA_LEN;
	u8 buf8[EEPROM_PAGE_SIZE];
	u8 *bin_buf;

	u32 start_page = IST30XX_EEPROM_SIZE / EEPROM_PAGE_SIZE - 8;
	u32 start_addr;

	tsp_info("Read compare start!!\n");

	ret = ist30xxb_isp_enable(true);
	if (unlikely(ret)) return ret;

	bin_buf = buf + ((start_page - page_size) * EEPROM_PAGE_SIZE);

	for (i = page_size; i > 0; i--) {
		start_addr = (start_page - i) * EEPROM_PAGE_SIZE;
		ist30xxb_isp_read(client, start_addr, (u32 *)buf8, len);
#if 0
		for (j = 0; j < EEPROM_PAGE_SIZE; j += 16) {
			tsp_debug("%07x: %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x %02x%02x\n", 
			  start_addr + j,
			  buf8[j], buf8[j + 1], buf8[j + 2], buf8[j + 3],
			  buf8[j + 4], buf8[j + 5], buf8[j + 6], buf8[j + 7],
			  buf8[j + 8], buf8[j + 9], buf8[j + 10], buf8[j + 11],
			  buf8[j + 12], buf8[j + 13], buf8[j + 14], buf8[j + 15]);
		}
#endif
	...
	ist30xx_cmd_run_device(client, true);
...
void ist30xx_set_tsp_fw(struct ist30xx_data *data)
{
	...
	case TSP_TYPE_SYNOPEX1:
		str = "SYNOPEX_5050";
		fw->buf = (u8 *)ist30xxb_fw_5050;
		fw->buf_size = sizeof(ist30xxb_fw_5050);
		break;
	...
	if ((fw->param_ver > 0) && (fw->param_ver < 0xFFFFFFFF)) {
		...
		if ((fw->core_ver == ist30xx_fw_ver) &&
			...
			if ((ret) || (chksum != fw->chksum)) {
				...
				retry = IST30XX_FW_UPDATE_RETRY;
				while (retry--) {
					ret = ist30xxb_read_compare(data->client, data->fw.buf, 1);
					if (ret == 0)
						break;
...
update_bin:   // TSP is not ready / FW update
	...
	if (retry > 0 && (ist30xx_check_fw(data, fw->buf) 
			|| ist30xxb_read_compare(data->client, data->fw.buf, 1)))
		goto update_bin;
