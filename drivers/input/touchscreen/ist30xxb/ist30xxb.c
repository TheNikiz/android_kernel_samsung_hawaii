...
struct delayed_work work_touch_booster;
struct mutex touch_booster_lock;

#define TOUCH_BOOSTING_DURATION		1000

static int touch_boosting = 0;
#endif

#define MAX_ERR_CNT             (100)

#define TOUCH_BOOSTER_SHMOBILE  0
...
static void start_touch_boosting(void)
{
	if( touch_boosting )
		return;
	
	pr_info("[TSP] Start touch boosting \n");	
		
	mutex_lock(&touch_booster_lock);

	cancel_delayed_work(&work_touch_booster);
	
	cpufreq_update_universal_lmt_req(BITMAP_KERNEL_SPACE_DVFS_TOUCH, 
				BOOST_MIN_CLOCK, MIN_LIMIT);

	touch_boosting = 1;
	
	schedule_delayed_work(&work_touch_booster, msecs_to_jiffies(TOUCH_BOOSTING_DURATION));
	
	mutex_unlock(&touch_booster_lock);
}

/*
		stop_touch_boosting() should be called for early suspend
 */
static void stop_touch_boosting(void)
{
	pr_info("[TSP] Stop touch boosting \n");			
	
	mutex_lock(&touch_booster_lock);

	if(touch_boosting)
		cpufreq_update_universal_lmt_req(BITMAP_KERNEL_SPACE_DVFS_TOUCH,
				RESTORE_MIN_CLOCK, MIN_LIMIT);
...
#if TOUCH_BOOSTER
	mutex_init(&touch_booster_lock);
	INIT_DELAYED_WORK(&work_touch_booster, stop_touch_boosting);
#if 0
	pi_mgr_qos_add_request(&touch_qos_node, "ist30xx",
			PI_MGR_PI_ID_ARM_CORE, PI_MGR_QOS_DEFAULT_VALUE);
	if (pi_mgr_dfs_add_request(&touch_dfs_node, "ist30xx",
				PI_MGR_PI_ID_ARM_CORE, PI_OPP_SUPER_TURBO))
		pr_err("Unable to add dfs request for zinitix\n");	
#endif
#endif
