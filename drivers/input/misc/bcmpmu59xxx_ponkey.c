...
#ifdef SEC_POWER_KEY_FACTORY
extern struct class *sec_class;
static struct device *sec_power_key_dev;
static ssize_t key_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bcmpmu_get_ponkey_state());
}
static DEVICE_ATTR(sec_power_key_pressed, S_IRUGO, key_show, NULL);
#endif
...
static int bcmpmu59xxx_ponkey_probe(struct platform_device *pdev)
{
	...
	sec_power_key_dev = device_create(sec_class, NULL, 0, NULL, "sec_power_key");
	if(IS_ERR(sec_power_key_dev))
	{
		pr_err("%s : Failed to create device for sysfs \n", __func__);
		return -EINVAL;
	}

	device_create_file(sec_power_key_dev, &dev_attr_sec_power_key_pressed);
#endif
