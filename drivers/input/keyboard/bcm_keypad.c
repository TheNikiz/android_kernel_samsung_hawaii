...
static void bcm_keypad_report_key_status(struct bcm_keypad *bcm_kb)
{
	struct bcm_keymap *keymap_p = bcm_kb->kpmap;
	CHAL_KEYPAD_REGISTER_SET_t event;
	cUInt32 numKeyEvt = 0, i;

	chal_keypad_retrieve_key_event_registers(keypadHandle, &event);

	pr_debug("[KEYPAD] %s, ssr0=0x%08x, ssr1=0x%08x\n",
		 __func__, event.ssr0, event.ssr1);

	numKeyEvt =
		chal_keypad_decode_pressed_key(keypadHandle, &event, keyEventList);

	for (i = 0; i < numKeyEvt; i++) {
		if (keyEventList[i].keyAction == CHAL_KEYPAD_KEY_PRESS) {
			pr_debug("[KEYPAD] %s, pressed key No.%d: %d, %s\n",
				__func__, i, keyEventList[i].keyId,
				keymap_p[keyEventList[i].keyId].name);
...
static int bcm_keypad_probe(struct platform_device *pdev)
{
	...
	bcm_keypad_report_key_status(bcm_kb);
...
int bcm_keypad_key_pressed(void)
{
       	unsigned int keyreadstatus1;
	unsigned int keyreadstatus2;
	...
	keyreadstatus2 = chal_keypad_config_read_status2();
        ...
        return (keyreadstatus1 || keyreadstatus2);

}
EXPORT_SYMBOL(bcm_keypad_key_pressed);
