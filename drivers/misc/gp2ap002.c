...
static ssize_t prox_open_calibration_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    int isOpenCal = 0;
    char cal_mode = 0;
    printk(KERN_INFO "[GP2A] %s\n", __func__);

    err = gp2a_cal_mode_read_file(&cal_mode);
    if (err < 0 && err != -ENOENT) {
        isOpenCal = 0;
...
static DEVICE_ATTR(prox_cal_open, S_IRUGO, prox_open_calibration_show, NULL);
...
static int gp2a_prox_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
	...
	reg_value = 0x00;
	ret = gp2a_i2c_write(GP2A_REG_PROX, &reg_value);
	if (ret < 0) 
        {
...
static int __init gp2a_prox_init(void)
{
	...
	if (device_create_file(dev_t, &dev_attr_prox_cal_open) < 0) 
		printk(KERN_ERR "Failed to create device file(%s)!\n", dev_attr_prox_cal_open.attr.name);
