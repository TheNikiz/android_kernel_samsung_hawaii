...
void bcmpmu_client_power_off(void)
{
	...
	pwr_mgr_pmu_reg_read_direct((u8) DEC_REG_ADD(PMU_REG_HOSTCTRL1),
				bcmpmu_get_slaveid(bcmpmu_gbl,
					PMU_REG_HOSTCTRL1),
				&val);

	pr_info("%s slave_id:%d [%d] val:%02x\n", __func__,
				bcmpmu_get_slaveid(bcmpmu_gbl, PMU_REG_WRLOCKKEY),
				bcmpmu_get_slaveid(bcmpmu_gbl, PMU_REG_HOSTCTRL1),
				val);
