...
uint8_t rt8973_get_otg_status(void)
{
	if(current_status !=NULL)
		return current_status->otg_status;  
	else
		printk(" current_status is NULL \n");
	return 0;
}
EXPORT_SYMBOL(rt8973_get_otg_status);
					...
					unsigned int new_status)
{
	...
	if(!chip->curr_status.ovp_status){
		if( chip->curr_status.cable_type ==MUIC_RT8973_CABLE_TYPE_L_USB)
		{		
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (ret < 0) {
		RTERR("Error : failed to request irq %d (gpio=%d, retval=%d)\n",
