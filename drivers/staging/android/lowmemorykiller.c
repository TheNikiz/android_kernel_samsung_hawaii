...
#ifdef CONFIG_PROTECT_HOME_LMK
pid_t protect_task_pid;
static int lmk_protect_write(struct file *file, const char __user * buffer,
				size_t count, loff_t * offs)
{

	protect_task_pid=current->pid;
	printk("launcer is forked. and protected pid : %d\n",protect_task_pid);
	return count;
}

static const struct file_operations lmk_protect_proc_fops = {
	.write = lmk_protect_write,
};

static void protect_tasks_lmk_init(void)
{
	proc_create("lmk_protect", S_IWUGO, NULL,
		    &lmk_protect_proc_fops);
	return;
}
#endif 
...
#ifdef CONFIG_PROTECT_HOME_LMK
		if(tsk->pid==protect_task_pid)
		{	
			if ( lowmem_adj[1] <min_score_adj)
				continue;
			else
				protect_task_pid=0;
		}
