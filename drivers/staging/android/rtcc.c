...
#define REMAIN_FREESWAP_SIZE 	1000 /* 4MB */
...
static int rtcc_thread(void * nothing)
{
	...
	for ( ; ; ) {
		...
		if (likely(atomic_read(&krtccd_running) == 1)) {
			...
			trace_mark_begin("rtcc_reclaim_pages", "");
#endif
			nr_reclaimed = rtcc_reclaim_pages(nr_to_reclaim, 200, &nr_swapped);
#if defined(CONFIG_TRACE_MARK)
			trace_mark_int(current->pid, "compressed_ratio", 0,"");
			trace_mark_end("rtcc_reclaim_pages");
#endif
			...
			} else if (si.freeswap < REMAIN_FREESWAP_SIZE) {
				rtcc_boost_mode = 0;
				printk("No swap space, swapped %ldMB, exit boost mode.\n", get_swapped_pages()/256);
			}
...
static int rtcc_idle_handler(struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if(si.freeswap < REMAIN_FREESWAP_SIZE)
		return 0;
