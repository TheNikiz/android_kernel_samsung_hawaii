...
*
* @file drivers/video/broadcom/lcd/heatnfc_dsi.c
*
...
struct heatnfc_dsi_lcd *lcd = NULL;
u8 gPanelID[3];
int multi_lcd_num = 2;
...
#define PANEL1_ID_CHECK_CONDITION (id1==0x55)&&(id2==0xBC)&&(id3==0xC0)
#define PANEL2_ID_CHECK_CONDITION (id1==0x55)&&(id2==0xBC)&&((id3==0xD0)|(id3==0xD1))

static int panel_read_id_verify ()
{
	int ret=-1;
	if(PANEL1_ID_CHECK_CONDITION){ // DTC
		ret=LCD_PANEL_ID_TWO;
		printk("[LCD] DTC panel, ret=%d\n", ret);
	}
	if(PANEL2_ID_CHECK_CONDITION){ // BOE
...
int panel_read_id(char *panel_name)
{
	...
	if(!panel1_id_checked){
		if((id1==0x55)&&(id2==0xBC)&&((id3==0xD0)|(id3==0xD1))){ // BOE	
			ret=LCD_PANEL_ID_ONE;
			printk("[LCD] BOE panel, ret=%d\n", ret);
		}
		else{
	...
	if(!panel2_id_checked){
		if((id1==0x55)&&(id2==0xBC)&&(id3==0xC0)){ // DTC
			ret=LCD_PANEL_ID_TWO;
			printk("[LCD] DTC panel, ret=%d\n", ret);			
		}
		else{
...
static ssize_t show_lcd_info(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(id3==0xD1)
	sprintf(temp, "BOE_55BCD1\n");
	else if(id3==0xC0)
	sprintf(temp, "BOE_55BCC0\n");	
...
{
	struct heatnfc_dsi_lcd *lcd = data;
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void heatnfc_dsi_early_suspend(struct early_suspend *earlysuspend)
{
  struct heatnfc_dsi_lcd *lcd = container_of(earlysuspend, struct heatnfc_dsi_lcd, earlysuspend);
...
static void heatnfc_dsi_late_resume(struct early_suspend *earlysuspend)
{
  struct heatnfc_dsi_lcd *lcd = container_of(earlysuspend, struct heatnfc_dsi_lcd, earlysuspend);
...
static int heatnfc_panel_probe(struct platform_device *pdev)
{
	int ret = 0;

	lcd = kzalloc(sizeof(struct heatnfc_dsi_lcd), GFP_KERNEL);
	if (!lcd)
		return -ENOMEM;
	...
	lcd->earlysuspend.suspend = heatnfc_dsi_early_suspend;
	lcd->earlysuspend.resume  = heatnfc_dsi_late_resume;
...
static int heatnfc_panel_remove(struct platform_device *pdev)
{
	int ret = 0;
		
	struct heatnfc_dsi_lcd *lcd = platform_get_drvdata(pdev);
...
static struct platform_driver heatnfc_panel_driver = {
	.driver		= {
		.name	= "heatnfc",
		.owner	= THIS_MODULE,
	},
	.probe		= heatnfc_panel_probe,
	.remove		= heatnfc_panel_remove,
};
static int __init heatnfc_panel_init(void)
{
	return platform_driver_register(&heatnfc_panel_driver);
}

static void __exit heatnfc_panel_exit(void)
{
	platform_driver_unregister(&heatnfc_panel_driver);
}

late_initcall_sync(heatnfc_panel_init);
module_exit(heatnfc_panel_exit);

MODULE_DESCRIPTION("heatnfc panel control driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:heatnfc");
