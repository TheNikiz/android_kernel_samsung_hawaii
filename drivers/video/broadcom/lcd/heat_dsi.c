...
*
* @file drivers/video/broadcom/lcd/heat_dsi.c
*
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void heat_dsi_early_suspend(struct early_suspend *earlysuspend)
{
  struct heat_dsi_lcd *lcd = container_of(earlysuspend, struct heat_dsi_lcd, earlysuspend);
...
static void heat_dsi_late_resume(struct early_suspend *earlysuspend)
{
  struct heat_dsi_lcd *lcd = container_of(earlysuspend, struct heat_dsi_lcd, earlysuspend);
...
static int heat_panel_probe(struct platform_device *pdev)
{
	int ret = 0;

	lcd = kzalloc(sizeof(struct heat_dsi_lcd), GFP_KERNEL);
	if (!lcd)
		return -ENOMEM;
	...
	lcd->earlysuspend.suspend = heat_dsi_early_suspend;
	lcd->earlysuspend.resume  = heat_dsi_late_resume;
...
static int heat_panel_remove(struct platform_device *pdev)
{
	int ret = 0;
		
	struct heat_dsi_lcd *lcd = platform_get_drvdata(pdev);
...
static struct platform_driver heat_panel_driver = {
	.driver		= {
		.name	= "heat",
		.owner	= THIS_MODULE,
	},
	.probe		= heat_panel_probe,
	.remove		= heat_panel_remove,
};
static int __init heat_panel_init(void)
{
	return platform_driver_register(&heat_panel_driver);
}

static void __exit heat_panel_exit(void)
{
	platform_driver_unregister(&heat_panel_driver);
}

late_initcall_sync(heat_panel_init);
module_exit(heat_panel_exit);

MODULE_DESCRIPTION("heat panel control driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform:heat");
