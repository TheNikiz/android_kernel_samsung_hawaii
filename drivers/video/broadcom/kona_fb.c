...
int g_lcd_id_num;
extern int panel_read_id(char *panel_name);
extern int sBoot_panel_read_id(char *boot_id);
...
#if !defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G) &&  !defined(CONFIG_MACH_HAWAII_SS_VIVALTODS5M)
...
static int __init lcd_panel_setup(char *panel)
{
	if (panel && strlen(panel)) {
		...
		g_lcd_id_num = sBoot_panel_read_id(panel);
	} else {
		pr_err(" lcd_panel_setup : NO PANEL ID from bootloader\n");
		g_disp_str[0] = 0xFF;
		g_lcd_id_num = 0;
	}
	g_display_enabled = 0;
...
static int __ref kona_fb_probe(struct platform_device *pdev)
{
	...
	g_num_of_fb_probe++;
	if (g_lcd_id_num >0 && g_lcd_id_num != g_num_of_fb_probe) {
		konafb_info("LCD id is not matched!!!\n");
		return -EINVAL;
	}
