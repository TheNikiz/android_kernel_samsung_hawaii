...
static int bsc_wait_cmdbusy(struct bsc_i2c_dev *dev)
{
	...
	if (time >= limit) {
		...
		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_CS_OFFSET);
		dev_err(dev->device, "CS = 0x%08x\n", tmp);

		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_IER_OFFSET);
		dev_err(dev->device, "IER = 0x%08x\n", tmp);

		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_ISR_OFFSET);
		dev_err(dev->device, "ISR = 0x%08x\n", tmp);

		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_CLKEN_OFFSET);
		dev_err(dev->device, "CLKEN = 0x%08x\n", tmp);

		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_TXCOUNT_OFFSET);
		dev_err(dev->device, "TXCOUNT = 0x%08x\n", tmp);

		tmp = BSC_READ_REG(dev->virt_base + I2C_MM_HS_FIFO_STATUS_OFFSET);
		dev_err(dev->device, "FIFO_STATUS = 0x%08x\n", tmp);


		status = bsc_read_intr_status((uint32_t)dev->virt_base);
		dev_err(dev->device, "INTR_STATUS = 0x%08x\n", status);

		/* do reset */
		dev_err(dev->device, "Reset BSC!!\n");
		queue_work(dev->reset_wq, &dev->reset_work);
	...
	if (atomic_read(&dev->polling_mode)) {
		...
		if (time_left == 0 ||
		   (dev->err_flag & I2C_MM_HS_ISR_ERR_MASK)) {
			dev_err(dev->device, "%s:controller timed out\n",__func__);
					      ...
					      unsigned int len)
{
	 ...
	 */
	time_left = wait_for_completion_timeout(&dev->rx_ready,
						dev->ses_timeout);
	bsc_disable_intr((uint32_t)dev->virt_base,
			 I2C_MM_HS_IER_READ_COMPLETE_INT_MASK);
	if (time_left == 0) {
...
static int bsc_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.platform_data) {
		...
		/* Set session timeout to its default value */
		dev->ses_timeout = msecs_to_jiffies(100);
		...
		if (of_property_read_u32(pdev->dev.of_node, "ses-timeout",
			&val))
			dev->ses_timeout = msecs_to_jiffies(100);
		else
			dev->ses_timeout = msecs_to_jiffies(val);
	...
	dev_info(dev->device, "bus %d at ses_timeout %d\n", pdev->id, dev->ses_timeout);
