...
#define SDHCI_MMC_CAP_UHS_ALL	(MMC_CAP_UHS_SDR12 | MMC_CAP_UHS_SDR25 | \
	...
	if (!host->data) {
		...
		if (host->cmd && (host->cmd->flags & MMC_RSP_BUSY)) {
			...
			if (intmask & SDHCI_INT_DATA_TIMEOUT &&
				...
				if ((sdhci_readl(host, SDHCI_PRESENT_STATE) &
					SDHCI_DATA_LVL_DAT0_MASK) == 0) {
					//mod_timer(&host->timer, jiffies + 12 * HZ);  //add 12 second delay to host->timer.
...
/* FPGA not fast enough for high speed */
	...
	if (host->quirks2 & SDHCI_QUIRK2_HOST_DISABLE_UHS)
		mmc->caps &= ~SDHCI_MMC_CAP_UHS_ALL;
