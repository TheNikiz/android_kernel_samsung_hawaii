...
static void GetLogFileName(char *buf, char *rootdev, int size)
{
	...
	snprintf(buf, size,
		 "%sCPLOG_filt_log_mtt_%d_%02d_%02d_%02d_%02d_%02d.bin",
		 rootdev,
		 tm.tm_year + 1900,
...
int WriteToLogDev_MODEMLOG(void)
{
	...
	pfile = 0;
	printk("++++WriteToLogDev_MODEMLOG start ++++\n");
	oldfs = get_fs();
	set_fs(KERNEL_DS);
	 ...
	 */
		...
		printk("++++WriteToLogDev_MODEMLOG GetLogFileName ++++\n");

		GetLogFileName(fname, "/data/log/err/", sizeof(fname));

		pfile = filp_open(fname, O_WRONLY | O_TRUNC | O_CREAT, 0664);
	...
	set_fs(oldfs);
		printk("---- WriteToLogDev_MODEMLOG finish ----\n");
	return 0;
}
