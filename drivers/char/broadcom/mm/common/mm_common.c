...
#ifdef CONFIG_ION
static struct ion_client *mm_gralloc_client;
DEFINE_MUTEX(mm_gralloc_mutex);

void mm_fmwk_alloc_buffer(struct mm_common *common,
				mm_gralloc_alloc_t *requested,
				mm_gralloc_alloc_t *allocated)
{
	unsigned int heap_mask;
	BUG_ON(mm_gralloc_client == NULL);
	if (!allocated->handle) {
		allocated->size = requested->size;
		allocated->flags = requested->flags;
		heap_mask = bcm_ion_get_heapmask(allocated->flags);
		allocated->handle = ion_alloc(mm_gralloc_client,
						allocated->size,
						0,
						heap_mask,
						allocated->flags);
		if (allocated->handle)
			allocated->da = bcm_ion_map_dma(mm_gralloc_client,
							allocated->handle);
		else
			pr_err("gralloc failed size[0x%x] and flags[0x%x]",
						allocated->size,
						allocated->flags);
		}

	if (allocated->handle) {
		requested->fd = ion_share_dma_buf_fd(mm_gralloc_client,
							allocated->handle);
		requested->da = allocated->da;
		requested->handle = allocated->handle;
		return;
	}
}
#endif
			...
			if (wait_job->job.type != INTERLOCK_WAITING_JOB) {
				list_for_each_entry_safe(wait_job, \
					...
					if (wait_job->job.type !=
						...
						common->mm_core[core_id]);
					else if (wait_job->predecessor == NULL)
						continue;
					else
						break;
					}
...
#endif
#ifdef CONFIG_ION
	private->hwc_alloc.fd = -1;
	private->gl_alloc.fd = -1;
	private->sw_alloc.fd = -1;
	private->enc_alloc.fd = -1;
#endif
	pr_debug(" %p ", private);
...
#endif
#ifdef CONFIG_ION
	if (private->hwc_alloc.handle)
		ion_free(mm_gralloc_client, private->hwc_alloc.handle);
	if (private->gl_alloc.handle)
		ion_free(mm_gralloc_client, private->gl_alloc.handle);
	if (private->sw_alloc.handle)
		ion_free(mm_gralloc_client, private->sw_alloc.handle);
	if (private->enc_alloc.handle)
		ion_free(mm_gralloc_client, private->enc_alloc.handle);
#endif
			...
			size_t size, loff_t *offset)
{
	...
	if (size > 0) {
		...
		if (!job_post) {
			pr_err("job_post pointer is NULL. kmalloc failure");
			goto out;
		}
			...
			unsigned long arg)
{
...
#endif
#ifdef CONFIG_ION
	case MM_IOCTL_GET_HWC_BUFFER:
	{
		mm_gralloc_alloc_t requested;
		if (copy_from_user(&requested,
					(void const *)arg,
					sizeof(requested))) {
			pr_err("copy_from_user failed");
			ret = -EINVAL;
		...
		requested.fd = -1;
		requested.da = 0;
		WARN_ON(requested.size > 0x1000000);
		mutex_lock(&mm_gralloc_mutex);
		mm_fmwk_alloc_buffer(common, &requested, &private->hwc_alloc);
		if (copy_to_user((void *) arg, &requested, sizeof(requested))) {
			pr_err("copy_to_user failed");
			ret = -EFAULT;
			}
		mutex_unlock(&mm_gralloc_mutex);
	}
	break;
	case MM_IOCTL_GET_GL_BUFFER:
	{
		mm_gralloc_alloc_t requested;
		if (copy_from_user(&requested,
					(void const *)arg,
		...
		WARN_ON(requested.size > 0x1000000);
		mutex_lock(&mm_gralloc_mutex);
		mm_fmwk_alloc_buffer(common, &requested, &private->gl_alloc);
		if (copy_to_user((void *) arg, &requested, sizeof(requested))) {
			pr_err("copy_to_user failed");
			ret = -EFAULT;
			}
	...
	case MM_IOCTL_GET_SW_BUFFER:
	{
		...
		WARN_ON(requested.size > 0x1000000);
		mutex_lock(&mm_gralloc_mutex);
		mm_fmwk_alloc_buffer(common, &requested, &private->sw_alloc);
		if (copy_to_user((void *) arg, &requested, sizeof(requested))) {
			pr_err("copy_to_user failed");
			ret = -EFAULT;
			}
	...
	case MM_IOCTL_GET_ENC_BUFFER:
	{
		...
		WARN_ON(requested.size > 0x1000000);
		mutex_lock(&mm_gralloc_mutex);
		mm_fmwk_alloc_buffer(common, &requested, &private->enc_alloc);
		if (copy_to_user((void *) arg, &requested, sizeof(requested))) {
			pr_err("copy_to_user failed");
			ret = -EFAULT;
			}
						...
						MM_PROF_HW_IFC *prof_param)
{
	...
	if (!common->mm_common_ifc.mm_name) {
		pr_err("common->mm_common_ifc.mm_name pointer is NULL. kmalloc failure");
		goto err_register;
	}
...
#ifdef CONFIG_ION
	if (idev && (mm_gralloc_client == NULL)) {
		mm_gralloc_client = ion_client_create(idev, "gralloc");
		}
#else
#error "MM Drivers Cannot work without ION"
#endif
