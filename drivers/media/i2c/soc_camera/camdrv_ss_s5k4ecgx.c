...
static int camdrv_ss_s5k4ecgx_set_ae_stable_status(struct v4l2_subdev *sd, unsigned short value);//Nikhil
static int camdrv_ss_s5k4ecgx_get_ae_stable_status_value(struct v4l2_subdev *sd);//Nikihl
int camdrv_ss_s5k4ecgx_set_preflash_sequence(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl);
...
#if defined(CONFIG_MACH_HAWAII_SS_HEAT3G) \
...
static struct regulator *VTCAM_IO_1_8_V; //Added for VT camera
#endif
...
/* individual configuration */
	...
	|| defined(CONFIG_MACH_HAWAII_SS_HEAT_REV00) \
...
#elif defined(CONFIG_MACH_HAWAII_SS_HEAT3G) \
	|| defined(CONFIG_MACH_HAWAII_SS_HEATNFC3G) || defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G) || defined(CONFIG_MACH_HAWAII_SS_VIVALTODS5M) \
...
#define VCAM_CORE_1_2V_REGULATOR	"vsrldo"
#define VCAM_AF_2_8V_REGULATOR		"mmcldo2"
#define VTCAM_IO_1_8V_REGULATOR		"lvldo2" //Front camera core power
#define VCAM_A_2_8V_REGULATOR_uV	2800000
#define VCAM_IO_1_8V_REGULATOR_uV	1786000
#define VCAM_CORE_1_2V_REGULATOR_uV	1200000
#define VCAM_AF_2_8V_REGULATOR_uV	2800000
#define VTCAM_IO_1_8V_REGULATOR_uV	1786000 ////Front camera core power
#define CAM0_RESET			111
#define CAM0_STNBY			2
#define CAM1_RESET			5	
#define CAM1_STNBY			4
...
static const struct v4l2_ctrl_config s5k4ecgx_controls[] = {
	{
...
#if !defined( CONFIG_MACH_HAWAII_SS_HEAT3G) && !defined(CONFIG_MACH_HAWAII_SS_HEATNFC3G) && !defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G) && !defined(CONFIG_MACH_HAWAII_SS_VIVALTODS5M)
	{
		.ops    = &camdrv_ss_ctrl_ops,
...
static int camdrv_ss_s5k4ecgx_set_capture_start(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
	int err = 0;
...
#endif
//denis
static int camdrv_ss_s5k4ecgx_set_scene_mode(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
	int err = 0;
...
/*NIKHIL*/
static int camdrv_ss_s5k4ecgx_set_ae_stable_status(struct v4l2_subdev *sd, unsigned short value)
{ 
 struct i2c_client *client = v4l2_get_subdevdata(sd);
    int err = 0;
...
#endif
static int camdrv_ss_s5k4ecgx_get_ae_stable_status(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{   
#if 0
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    int err = 0;
...
static int camdrv_ss_s5k4ecgx_get_auto_focus_status(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{ 
    struct i2c_client *client = v4l2_get_subdevdata(sd);
    struct camdrv_ss_state *state = to_state(sd);
    int err = 0;
...
static int camdrv_ss_s5k4ecgx_get_touch_focus_status(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
...
static int camdrv_ss_s5k4ecgx_set_auto_focus(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	struct camdrv_ss_state *state = to_state(sd);
	int err = 0;
...
static int camdrv_ss_s5k4ecgx_sensor_power(struct v4l2_subdev *sd, int on)
{
	unsigned int value;
	int ret = -1;
	struct clk *clock;
	...
	VTCAM_IO_1_8_V  = regulator_get(NULL, VTCAM_IO_1_8V_REGULATOR);
	if(IS_ERR(VTCAM_IO_1_8_V))
	{
...
//		iowrite32(value, padctl_base + PADCTRLREG_DCLK1_OFFSET);
	...
	if(on)
	{
		...
		if (value)
            ...
            //Added Handler for VT camera
           #if defined(CONFIG_MACH_HAWAII_SS_HEAT3G) \
	                   || defined(CONFIG_MACH_HAWAII_SS_HEATNFC3G) || defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G) || defined(CONFIG_MACH_HAWAII_SS_VIVALTODS5M)
	
             value = regulator_set_voltage(VTCAM_IO_1_8_V, VTCAM_IO_1_8V_REGULATOR_uV, VTCAM_IO_1_8V_REGULATOR_uV);
		if (value)
			CAM_ERROR_PRINTK("%s:regulator_set_voltage VCAM_IO_1_8_V failed \n", __func__);
		...
		value = regulator_enable(VTCAM_IO_1_8_V);
		if (value) {
			CAM_ERROR_PRINTK("%s:failed to enable VTCAM_IO_1_8_V\n", __func__);
			return -1;
		}
...
//#ifdef CONFIG_FLASH_ENABLE
	...
	else
	{
		...
		gpio_set_value(CAM0_RESET,0);
              /*
                These clock disable setting will take care delay
                 so delays not needed during the Reset , the axi/lp clock disable and standby.
              */
...
//		gpio_set_value(CAM_AF_EN,0); 
		...
		msleep(1);
             ...
             regulator_disable(VTCAM_IO_1_8_V);
...
{
    strcpy(rear_camera_vendorid,"0x0901");
}
