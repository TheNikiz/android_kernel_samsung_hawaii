...
int flash_check =0;
int torch_check=0; //sanjeev.garg
int flagforAE=0;
...
static int camdrv_ss_actual_sensor_power_up(struct v4l2_subdev *sd);
...
static struct device_attribute dev_attr_rear_flash = {
	.attr = {
		.name = "rear_flash",
		.mode =(S_IWUSR|S_IWGRP|S_IROTH)},     //( S_IWUSR|S_IWGRP|S_IROTH)},
...
{
	//int count;
    CAM_INFO_PRINTK("%s : rear_vendorid is \n", __func__);
...
typedef struct regs_array_type {
	...
	unsigned short mask; // for ISX012_REGISTER_SIZE
} regs_short_t;
...
static reg_hash_t reg_hash_table[] = {
	...
	{"reg_main_callibration_shading_table_nocal",								NULL},
	{"reg_main_callibration_shading_table_0",								NULL},
	{"reg_main_callibration_shading_table_1",								NULL},
	{"reg_main_callibration_shading_table_2",								NULL},
#endif	
	{"init_regs",								NULL},
	...
	{"fps_auto_regs",							NULL},
	{"fps_auto_normal_regs",							NULL},
	{"fps_auto_Dark_regs",							NULL},
	{"fps_5_regs",								NULL},
	{"fps_7_regs",								NULL},
	{"fps_10_regs",								NULL},
	{"fps_15_regs",								NULL},
...
#endif
#if defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G_REV00)
	{"preview_size_1280x720_regs",						NULL},
#endif
	...
	{"ae_lock_regs",							NULL},
	{"ae_unlock_regs",							NULL},
	{"ae_maual_mode_regs",					NULL},
	{"awb_lock_regs",							NULL},
	{"awb_unlock_regs",							NULL},
	{"iso_auto_regs",							NULL},
#if defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G_REV00)
	{"af_window_reset",							NULL},
	{"af_window_set",							NULL},
	{"af_opd4",							NULL},
#endif
	{"iso_50_regs",								NULL},
...
#if defined(CONFIG_MACH_HAWAII_SS_VIVALTONFC3G_REV00)
	{"halfrelease_mode",						NULL},
	{"lowlux_night_halfrelease_mode",						NULL},
	{"af_restart",						NULL},
	{"af_write",						NULL},
	{"af_saf_off",						NULL},
	{"af_touchsaf_off",						NULL},
#endif
...
char register_buf[7], data_buf[7]; /* , data_temp_buf[4]; */
unsigned char reg[10];
unsigned char reg_val1[10];
unsigned char reg_val2[10];
...
static int camdrv_ss_regs_parse_table(void)
{
	...
	memset(register_buf, '\0', strlen(register_buf));
	memset(data_buf, '\0', strlen(data_buf));
	memset(reg_val1, '\0', strlen(reg_val1));
	memset(reg_val2, '\0', strlen(reg_val2));
	...
	while (camdrv_ss_regs_get_line(current_line)) {
		...
		if (sensor.register_size == 4) {
			...
			if ((current_line[0] == '0') && (current_line[10] == ',')) {
				...
				memcpy(register_buf, (const void *)&current_line[0], 6);
				reg_table[reg_index].subaddr = (unsigned short)simple_strtoul(register_buf, NULL, 16);
		...
		else if (sensor.register_size == 0xFF) { //sanjeev.garg
			/* Parsing a register format : {0x0000,0x00,0x01} and {0x0000,0x0000,0x02}, */
			if ((current_line[0] == '{')&&(current_line[1] == '0') && (current_line[7] == ','))
			{
				memcpy(register_buf, (const void *)&current_line[1], 6);
				reg_table[reg_index].subaddr = (unsigned short)simple_strtoul(register_buf, NULL, 16);
				
				if(current_line[12]==',')
				{
					memcpy(reg_val1, (const void *)&current_line[8], 4);
					memcpy(data_buf, (const void *)&current_line[13], 4);

					reg_table[reg_index].value = (unsigned int)simple_strtoul(reg_val1, NULL, 16)&0x0000FFFF;
				}
				else if(current_line[14]==',')
				{
					
					memcpy(reg_val2, (const void *)&current_line[8], 6);
					memcpy(data_buf, (const void *)&current_line[15], 4);
					
					reg_table[reg_index].value = (unsigned int)simple_strtoul(reg_val2, NULL, 16);
				}
				else
				{
					CAM_ERROR_PRINTK("sanjeev.garg .. problem in parsing register header file ..");
				}
				
		
				reg_table[reg_index].mask= (unsigned short)simple_strtoul(data_buf, NULL, 16);

				//CAM_ERROR_PRINTK("%s, Parsed register values .. reg_table[%d].subaddr = 0x%x\n",__func__, reg_index, reg_table[reg_index].subaddr );
				//CAM_ERROR_PRINTK("%s, Parsed register values .. reg_table[%d].value = 0x%x\n",__func__, reg_index, reg_table[reg_index].value );
				//CAM_ERROR_PRINTK("%s, Parsed register values .. reg_table[%d].mask = 0x%x\n",__func__, reg_index, reg_table[reg_index].mask);
...
static int camdrv_ss_regs_table_write(struct i2c_client *client, char *name)
{
	...
	unsigned char dataLegth = 0, data_value8 = 0;
		...
		for (i = 0; i < reg_num_of_element; i++) {
			...
			if (subaddr8 == sensor.delay_duration) {
	...
	}else if(sensor.register_size == 0xFF){
				...
				for (i = 0; i < reg_num_of_element; i++)  {
					...
					subaddr =  reg_table[i].subaddr;
					data_value =  reg_table[i].value;
					dataLegth =  reg_table[i].mask;
//CAM_INFO_PRINTK("i=%d addr=%x val=%x mask=%x  num_of_regs=%d\n", i, reg_table[i].subaddr,reg_table[i].value,reg_table[i].mask,reg_num_of_element); 						
						if(dataLegth == 0x01) // 1 byte data
						{
							data_value8 = (data_value & 0xff);
							
							if (subaddr == sensor.delay_duration) {
								msleep(data_value8);
								CAM_INFO_PRINTK("%s :: register 3 size :: delay : delay_duration = 0x%x, real delay : %d ms\n",
									__func__, sensor.delay_duration, data_value8*10);
							} else {
							  	err = camdrv_ss_i2c_write_3_bytes(client, subaddr, data_value8); 
							} 	
						}
						else if(dataLegth == 0x02) // 2 bytes data
						{
							if (subaddr == sensor.delay_duration) {
								CAM_ERROR_PRINTK("%s %s :i2c Incorrect Delay data !\n", sensor.name, __func__);
								return -EIO;
							}
							else
							{
						...
						{
							CAM_ERROR_PRINTK("%s %s :i2c Data count over 2 bytes !\n", sensor.name, __func__);
							return -EIO;
						}
...
static int camdrv_ss_g_volatile_ctrl(struct v4l2_ctrl *ctrl);
static int camdrv_ss_try_ctrl(struct v4l2_ctrl *ctrl);

const struct v4l2_ctrl_ops camdrv_ss_ctrl_ops = {
	.g_volatile_ctrl = camdrv_ss_g_volatile_ctrl,
	.s_ctrl = camdrv_ss_s_ctrl,
	.try_ctrl = camdrv_ss_try_ctrl,
};

static int camdrv_ss_g_ctrl(struct v4l2_subdev *sd,
				...
				unsigned short *data)
{
	...
	int err = 0;
	CAM_ERROR_PRINTK("%s : client->adapter = %x!!!\n", sensor.name, (int)client->adapter);
...
int camdrv_ss_i2c_read_2_bytes_2(struct i2c_client *client,
				unsigned short subaddr,
				unsigned short *data)
{
	unsigned char buf[2];
...
int camdrv_ss_i2c_read_2_bytes_1(struct i2c_client *client,
				unsigned short subaddr,
				unsigned short *data)
{
	unsigned char buf[2];
...
/**************************************************************************
* camdrv_ss_i2c_write_4_bytes_reverse_data : Write (I2C) multiple bytes to the camera sensor
* @client: pointer to i2c_client
* @subaddr: register address
* @data: data to be written
*
* Returns 0 on success, <0 on error
***************************************************************************/
int camdrv_ss_i2c_write_4_bytes_reverse_data(struct i2c_client *client,
                                                                unsigned short subaddr,
                                                                unsigned short data)
{
                unsigned char buf[4];
...
/**************************************************************************
 * camdrv_ss_i2c_write_3_bytes: Write (I2C) multiple bytes to the camera sensor
 * @client: pointer to i2c_client
 * @subaddr: register address
 * @data: data to be written
 *
 * Returns 0 on success, <0 on error
 ***************************************************************************/
int camdrv_ss_i2c_write_3_bytes(struct i2c_client *client,
				unsigned short subaddr,
				unsigned short data)
{
	unsigned char buf[3];
...
#define BURSTWRITE_BUFFER	(2000)
unsigned char burst_buffer[BURSTWRITE_BUFFER];
					...
					char *name)
{
	...
	struct i2c_reg_value_1 *reg_buf = (struct i2c_reg_value_1*) reg_buffer;
#ifdef CONFIG_LOAD_FILE
	...
	if ((sensor.i2c_set_data_burst != NULL) && (strcmp(name, "init_regs") == 0)) {
		...
		for (i = 0; i < num_of_regs; i++) {
			 //CAM_INFO_PRINTK("%s :: reg_buffer[%d]=0x%x \n",__func__, i,reg_buffer[i]); 
			if (sensor.register_size == 4) {
			...
			{
				if (count != num_of_regs) {
					//CAM_INFO_PRINTK("i=%d addr=%x val=%x mask=%x  num_of_regs=%d\n", i, reg_buf->addr,reg_buf->value,reg_buf->mask,num_of_regs); 
					subaddr = reg_buf->addr;
					data_value = reg_buf->value ;
					dataLegth = reg_buf->mask;
						
					if(0xFFFD != subaddr)
					{
						if(burst_start == false)
						{
					        if (dataLegth == 0x02) 
					        {
					            if (subaddr == sensor.delay_duration) 
						...
						{
								CAM_ERROR_PRINTK("%s %s %s i=%d addr=0x%04x val=0x%04x mask=0x%02x  num_of_regs=%d\n", name, sensor.name, __func__, i, reg_buf->addr,reg_buf->value,reg_buf->mask,num_of_regs);  
								CAM_ERROR_PRINTK("%s %s %s :i2c Data count over 2 bytes (%d)!\n", name, sensor.name, __func__,dataLegth);
								return -EIO;
							}
								
					        if (err < 0) 
					        {
					            CAM_ERROR_PRINTK("ERR(%s):i2c write error i = %d\n", __func__, i);
					            return -1;
					        }
					    }
							else
							...
							if(burst_index == 0)
							{
								...
								burst_buffer[burst_index ++] = (subaddr >> 8) & 0xFF;
								//CAM_INFO_PRINTK("(%s) First byte Burst Address = 0x%02x\n", __func__, burst_buffer[burst_index-1]);
								burst_buffer[burst_index ++] = subaddr & 0xFF;
								//CAM_INFO_PRINTK("(%s) second Burst Address = 0x%02x\n", __func__, burst_buffer[burst_index-1]);
							}

							if (dataLegth == 0x02) 
							{
					         	burst_buffer[burst_index ++] = data_value & 0xFF;
								//CAM_INFO_PRINTK("(%s)(%d) Burst Data = 0x%02x\n", __func__, burst_index, burst_buffer[burst_index-1]);
					  			burst_buffer[burst_index ++] = (data_value >> 8) & 0xFF;
								//CAM_INFO_PRINTK("(%s)(%d) Butst Data = 0x%02x\n", __func__, burst_index, burst_buffer[burst_index-1]);

								if (BURSTWRITE_BUFFER <= (burst_index + 4))
								{
					                retry = 5;
									msg.buf = burst_buffer;
									msg.len = burst_index;
					                ...
					                if ((err < 0) || (retry <= 0)) {
					                    CAM_ERROR_PRINTK("Sensor I/O write failed. ret = %d, retry = %d\n", err, retry);
					                    return -EIO;
					                }
					        ...
					        else if (dataLegth == 0x01) 
					        {
					            burst_buffer[burst_index ++] = data_value & 0xFF;
								//CAM_INFO_PRINTK("(%s)(%d) Butst Data = 0x%02x\n", __func__, burst_index, burst_buffer[burst_index-1]);
								if (BURSTWRITE_BUFFER <= (burst_index + 3))
								{
					                retry = 5;
									msg.buf = burst_buffer;
									msg.len = burst_index;
			...
			{
				CAM_ERROR_PRINTK("%s %s :register size is not 4 or 2 or Variable bytes = %d !\n", sensor.name, __func__, sensor.register_size);
				return -EIO;
			}
...
void camdrv_ss_check_regulator_is_enabled(struct regulator *regul, const char *regulName)
{
   if (regul != NULL)
   {
      int retVal;
      ...
      /* Check if a regulator is enabled
             *    Positive  : if enabled
             *    Zero     : if not enabled
             *    Negative : Error happens
             */
      retVal = regulator_is_enabled(regul);
      if (retVal == 0)
      {
         CAM_INFO_PRINTK("WARN : Regulator (%s) is not yet enabled. Wait more\n", regulName);
         msleep(20);
      }
      else if (retVal < 0)
      {
         CAM_ERROR_PRINTK("ERROR : on Regulator (%s) (retVal = %d)\n", regulName, retVal);
      }
      else
      {
         CAM_INFO_PRINTK("SUCCESS : Regulator (%s) is enabled !!!\n", regulName);
      }
   }
}
...
static int camdrv_ss_set_vt_mode(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	...
	case CAM_VT_MODE_SMART_STAY:
	{
		...
		if (tmp_sensor_state == CAMDRV_SS_NOT_INITIALIZED) {
			...
			if (0 != camdrv_ss_actual_sensor_power_up(sd)) {
...
static int camdrv_ss_set_frame_rate(struct v4l2_subdev *sd, struct v4l2_ctrl *ctrl)
{
	...
	CAM_INFO_PRINTK("%s %s :camdrv_ss_set_frame_rate  E  = %d err=%d\n", sensor.name, __func__, ctrl->val,err);
...
static int camdrv_ss_set_focus_mode(struct v4l2_subdev *sd, int value)
{
	...
	case FOCUS_MODE_MACRO:
	{
		...
		if (sensor.af_restart == 0)
			CAM_ERROR_PRINTK("%s %s : af_restart not  supported !!!\n", sensor.name, __func__);
		else
			err = camdrv_ss_i2c_set_config_register(client, sensor.af_restart, sensor.rows_num_af_restart, "af_restart");
		...
		struct v4l2_control *v4l2Ctrl)
{
	...
	switch (v4l2Ctrl->id) {
		case V4L2_CID_CAMERA_READ_MODE_CHANGE_REG:
		{
			...
			/* but other case include error case, */
			/* just do capture without skip frame */
			CAM_INFO_PRINTK("%s %s : Not in CAPTURE MODE\n",
									sensor.name, __func__);
				v4l2Ctrl->value = 1;
			} else
			    /* CAMDRV_SS_CAPTURE_MODE_READY 
					or CAMDRV_SS_CAPTURE_MODE_NOT_SUPPORT */
			CAM_INFO_PRINTK("%s %s : CAPTURE MODE READY\n",
				sensor.name, __func__);
				v4l2Ctrl->value = 0;
			break;
		}

		case V4L2_CID_CAMERA_READ_CAM_MODE:
		v4l2Ctrl->value =
			(state->current_mode == PICTURE_MODE ? 1 : 0);
			break;
...
static int camdrv_ss_try_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	switch (ctrl->id) {
		...
		case V4L2_CID_CAMERA_FLASH_MODE:
			ctrl->cur.val = FLASH_MODE_BASE;
			break;

		case V4L2_CID_CAMERA_FLASH_CONTROL:
			ctrl->cur.val = FLASH_CONTROL_BASE;
			break;
		...
		case V4L2_CID_CAMERA_SAMSUNG:
			/* Samsung camera or 3rd party camera */
			ctrl->cur.val = 0;
			break;	
...
static int camdrv_ss_s_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	if (ctrl->id == V4L2_CID_CAM_SET_MODE) {
        ...
        if (ctrl->id == V4L2_CID_CAMERA_SAMSUNG)//case V4L2_CID_CAMERA_SAMSUNG:
	{
	    flagforAE=1;
            CAM_INFO_PRINTK( "%s %s :  flagforAE set =%d\n", sensor.name, __func__, flagforAE);
	    return 0;
	}
	...
	case V4L2_CID_CAMERA_BRIGHTNESS:
	{
		if (sensor.set_ev!= NULL) { 
			CAM_ERROR_PRINTK("%s %s set_ev :sensor has defined its sequence\n", sensor.name, __func__);
			sensor.set_ev(sd, ctrl->val);
		} else
	...
	mutex_unlock(&sensor_s_ctrl);
	 CAM_INFO_PRINTK("%s %s : %d SUCCESS err =%d \n",
			sensor.name, __func__, ctrl->id,err); 
	return err;
}
...
static int camdrv_ss_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if (sensor.init_regs == NULL)
		CAM_ERROR_PRINTK("%s %s : init_regs is NULL, please check if it is needed !!!\n", sensor.name, __func__);
			...
			if (sensor.checkwait_for_mode_transition != NULL) 
			{
				CAM_ERROR_PRINTK("%s wait_for_mode_transition in init \n", __func__);
				sensor.checkwait_for_mode_transition(sd, 1);
			}
			if (sensor.calibrateSensor != NULL) 
			{
				CAM_ERROR_PRINTK("%s First OM check\n", __func__);
				sensor.calibrateSensor(sd);
			}
...
static int camdrv_ss_subdev_power(struct v4l2_subdev *sd, int on)
{
	...
	if (!on)
	{
		if(camera_init_check == true)
		{			
			if(sensor.set_vcm_off != NULL) {
				CAM_ERROR_PRINTK("VCM Off before powered OFF\n");
				sensor.set_vcm_off(sd);
			}
			camera_init_check = false;
		}
								  ...
								  const struct i2c_device_id *did)
{
	...
	CAM_INFO_PRINTK("%s sensors_reg_main_count=%d :\n", __func__, sensors_reg_main_count);
