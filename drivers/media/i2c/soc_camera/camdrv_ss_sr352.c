...
#if defined(CONFIG_MACH_HAWAII_SS_KYLEVE_REV00) || defined(CONFIG_MACH_HAWAII_SS_JBTLP_REV00) || defined(CONFIG_MACH_HAWAII_SS_KYLEVESS_REV00)
...
*/ //already size setting is included in init registers. we don't need to write again.
   ...
   CAM_ERROR_PRINTK( "%s : [BRCM] Insert Delay", __func__);
	msleep(100);
...
static int camdrv_ss_sr352_sensor_power(struct v4l2_subdev *sd, int on)
{
	unsigned int value;
	int ret = -1;
	struct clk *clock;
	...
	gpio_request(CAM0_STNBY, "CAM_STNBY");
...
static int camdrv_ss_sr352_set_scene_mode(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	CAM_INFO_PRINTK( "%s : value =%d\n", __func__, ctrl->value);
		...
		{
				err = camdrv_ss_i2c_set_config_register(client, sr352_scene_beach_snow_regs, ARRAY_SIZE(sr352_scene_beach_snow_regs), "scene_sunset_regs");
			break;
		}
...
bool camdrv_ss_sensor_functions_sr352(struct camdrv_ss_sensor_cap *sensor)
{
...
int camdrv_ss_read_device_id_sr352(
...
static int __init camdrv_ss_sr352_mod_init(void)
{
	struct camdrv_ss_sensor_reg sens;

	strncpy(sens.name, sr352_NAME, sizeof(sr352_NAME));
	sens.sensor_functions = camdrv_ss_sensor_functions_sr352;
	sens.sensor_power = camdrv_ss_sr352_sensor_power;
	sens.read_device_id = camdrv_ss_read_device_id_sr352;
#ifdef CONFIG_SOC_CAMERA_MAIN_SR352
	sens.isMainSensor = 1;
#endif

#ifdef CONFIG_SOC_CAMERA_SUB_SR352
	sens.isMainSensor = 0;
#endif
	camdrv_ss_sensors_register(&sens);

}
module_init(camdrv_ss_sr352_mod_init);

MODULE_DESCRIPTION("SAMSUNG CAMERA SENSOR SR352 ");
