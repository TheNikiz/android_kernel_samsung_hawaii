...
#ifdef CONFIG_TRACE_MARK
static ssize_t
trace_mark_syscall_tgid_read(struct file *filp, char __user *ubuf,
	       size_t cnt, loff_t *ppos)
{
	char buf[64];

	int r = snprintf(buf, sizeof(buf) / sizeof(buf[0]), 
			"%ld\n", trace_mark_syscall_tgid);

	return simple_read_from_buffer(ubuf, cnt, ppos, buf, r);
}

static ssize_t
trace_mark_syscall_tgid_write(struct file *filp, const char __user *ubuf,
		size_t cnt, loff_t *ppos)
{
	int ret;

	if ((ret = kstrtoul_from_user(ubuf, cnt, 10, &trace_mark_syscall_tgid)))
		return ret;
...
static const struct file_operations trace_mark_syscall_tgid_fops = {
	.open		= tracing_open_generic_tr,
	.read		= trace_mark_syscall_tgid_read,
	.write		= trace_mark_syscall_tgid_write,
	.release	= tracing_release_generic_tr,
...
#ifdef CONFIG_TRACE_MARK
	trace_create_file("syscall_tgid", 0644, d_tracer,
			  tr, &trace_mark_syscall_tgid_fops);
#endif
